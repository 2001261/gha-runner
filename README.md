...
给 AI agent 配的**云端计件工人**（worker）。它有点像云沙盒，但不是沙盒——沙盒是
一台一直给你占着的机器，它是"来活才开机"：agent 把任务打包丢过去，GitHub 在云端
开一台干净的 4 核 16G 机器，跑完把结果（日志、产物文件、结构化报告）原样带回来，
机器当场销毁，下次来活再开新的。

它解决了什么：

- **小内存 / 小算力 agent 的救星**：本地跑不动的重活（编译、批处理、爬虫抓取、
  大规模分析、浏览器自动化）统统卸载到云端，本地只负责收发，agent 该干嘛干嘛
- **不占对话回合**：任务一交就返回，agent 不用干等进度条，好了再取结果——
  回合留给思考，不浪费在等待上
- **不要钱**：公开仓库的 GitHub Actions 分钟数免费不限量，4 vCPU / 16 GB /
  86 GB 磁盘随便用
- **力气大还能分身**：一个任务拆成上百份并行跑（matrix 最多 256 个 job），
  单机想都不敢想的吞吐
- **不怕机器被回收**：runner 到点销毁没关系，状态存缓存，下台机器从断点接着跑
- **凡事留证据**：每次运行都有完整日志 + 产物 + 结构化清单，结果能直接喂给
  agent 的下一步产物

一句话：**你的 agent 出脑子，GitHub 出机器。**
