...
license: Apache-2.0
metadata:
  short-description: agent 的免费云端 worker：重活丢到 GitHub Actions 上跑，结果取回来接着用
...
给 agent 配的**云端计件工人**（worker）。有点像云沙盒，但不是沙盒——沙盒是一台
一直占着的机器，它是"来活才开机"：把任务打包丢过去，GitHub 在云端开一台干净的
4 核 16G 机器，跑完把结果（日志、产物、结构化报告）带回来，机器当场销毁。

- **小内存 / 小算力 agent 的救星**：本地跑不动的重活（编译、批处理、抓取、
  大规模分析、浏览器自动化）卸载到云端，本地只负责收发
- **不占对话回合**：任务一交就返回，好了再取结果，回合留给思考而不是等进度条
- **不要钱**：公开仓库 Actions 分钟数免费不限量，4 vCPU / 16 GB / 86 GB 磁盘
- **能分身**：任务拆成上百份并行跑（matrix 最多 256 个 job）
- **不怕机器被回收**：状态存缓存，下台机器从断点接着跑
- **凡事留证据**：完整日志 + 产物 + 结构化清单，结果直接喂给后续产物

一句话：**agent 出脑子，GitHub 出机器。**
